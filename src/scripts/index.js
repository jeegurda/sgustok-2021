...
const getHtml = ({ image, id, video, url, status, background }) => {
  ...
  return `
      ${
        url
          ? `<a href="${url}" target="_blank" rel="noreferrer noopener" tabindex="-1"`
          : '<div'
      } class="swiper-slide project" data-hash="${id}"${style}>
        ${media}
        ${action}
      ${url ? '</a>' : '</div>'}
    `
...
const addTriggerEvents = (slider, content) => {
  ...
  slider.on('slideChange', () => {
    if (currentTextId !== null && currentTextId !== currentSlide()) {
      // Change only, don't open (TEMP: and don't close) description by just scrolling
